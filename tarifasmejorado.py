...
if all(file_dict.values()):
    try:
        ...
        if id_comercializador_col in tc1.columns and niu_col in tc1.columns:
            tc1_filtrado = tc1[tc1[id_comercializador_col] == 23442].copy()
        ...
        if niu_col in tc2.columns:
            ...
            tc2_sin_duplicados = tc2.drop_duplicates(subset=niu_col).copy()
    ...
    if all(col in tc1_filtrado.columns for col in required_columns):
        ...
        Tarifas.loc[:, 'NIU']     = Tarifas['NIU'].astype(str).str.strip()     # <-- parche .loc aquí
        Tarifas.loc[:, 'ESTRATO'] = (                                              
            Tarifas['ESTRATO']
                .replace({7:'I',8:'C',9:'O',11:'AP'})
                .astype(str)
        )
        ...
        # — Forzar columnas mixtas a string para evitar errores de PyArrow —
        for col in ['NIU','ESTRATO','TIPO TARIFA','UBICACION','DIVIPOLA','Municipio']:
            if col in Tarifas.columns:
                Tarifas.loc[:, col] = Tarifas[col].astype(str)
